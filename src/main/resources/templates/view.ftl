...
<form action="${request.contextPath}/countries/save" method="post">
    ...
    <table class="gridtable" style="width:800px;">
        ...
        <tr>
            <th>国家(地区)名称：</th>
            <td><input type="text" name="countryname" value="<#if country.countryname??>${country.countryname}</#if>"/></td>
            <th>国家(地区)代码：</th>
            <td><input type="text" name="countrycode" value="<#if country.countrycode??>${country.countrycode}</#if>"/></td>
